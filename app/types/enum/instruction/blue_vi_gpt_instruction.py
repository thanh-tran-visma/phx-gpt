...
class BlueViInstructionEnum(str, Enum):
    ...
    BLUE_VI_SYSTEM_HANDLE_INSTRUCTION_DECISION = (
        "Review the conversation history and classify the user's request in JSON format, "
        "with double quotes around both keys and values. Do not add extra fields, comments, or use single quotes.\n"
        "Classify the request as:\n"
        f"- {TrainingInstructionEnum.OPERATION_INSTRUCTION.value} for requests to create or modify an operation.\n"
        f"- {TrainingInstructionEnum.DEFAULT.value} for other types of requests.\n"
        "Determine CRUD (Create, Read, Update, Delete):\n"
        f"- If the request is classified as {InstructionList.DEFAULT.value}, set 'crud' to 'NONE'.\n"
        "- If the request is not classified as {InstructionList.DEFAULT.value}, 'crud' must be provided.\n"
        "Sensitive Data Assessment:\n"
        f"- If the request is classified as {InstructionList.DEFAULT.value}, evaluate the data for sensitive information.\n"
        "- If it is not {InstructionList.DEFAULT.value}, set 'sensitive_data' to False.\n"
        "Check for personal data under GDPR compliance:\n"
        "- **Who**: Does the request contain full names or identifiable details?\n"
        "- **Where**: Are there addresses or locations that can identify an individual?\n"
